...
public class SwaggerModelTransformer {
  ...
  /**
   * Apply all transformations to a Swagger model.
   *
   * @param swagger Swagger model to transform
   * @return Transformed Swagger model
   */
  public Swagger transform(Swagger swagger) {
    transformations.forEach(t -> t.beforeHook(swagger));

    for (Path path : MapUtils.map(swagger.getPaths()).values()) {
      for (Operation operation : ListUtils.list(path.getOperations())) {
